            ...
            <div class="shadow sm:rounded-md sm:overflow-hidden">
                <div class="px-4 py-5 bg-white space-y-6 sm:p-6">
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea id="about" name="title" rows="3" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md" placeholder="Type the title..." value="{{ old('title') }}">{{ $post->blog_title }}</textarea>
                            @error("title")
                    ...
                    <div>
                        ...
                        <div class="mt-1">
                            <textarea name="description" rows="10" class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm border border-gray-300 rounded-md" placeholder="Type the description..." value="{{ old('description') }}">{{ $post->blog_content }}</textarea>
                            @error("description")
                            <p class="text-red-500 text-xs italic">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        <div class="mt-1 flex rounded-md shadow-sm border border-r-0 border-gray-300">
                            ...
                            <input type="file" name="image" class="focus:ring-indigo-500 focus:border-indigo-500 flex-1 block w-full rounded-none rounded-r-md sm:text-sm border-gray-300" placeholder="{{ $post->blog_img }}">
                        </div>
                        @error("image")
                        <p class="text-red-500 text-xs italic">{{ $message }}</p>
                        @enderror
